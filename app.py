...
load_dotenv()
app = Flask(__name__)
app.secret_key = os.urandom(24) # Enables login memory (sessions)
...
# Using gemini-1.5-flash to fix the 404 error
model = genai.GenerativeModel('gemini-1.5-flash')

@app.route('/')
def index():
    # Passes session data to HTML to show/hide sections
...
def login():
    if request.method == 'POST':
        ...
        if u == ADMIN_DATA['username'] and p == ADMIN_DATA['password']:
            session['role'] = 'admin'
            return jsonify({"status": "success", "role": "admin", "message": "Admin recognized."})
